...
export const get_image_url_for_path = (path_uppercase: string) => {
  ...
  if (icon_cached) {
    ...
    console.warn(`Generating icon for '${path}'`);
    let thing = iconImage.get_icon_for_path(path);
    const native_icon = nativeImage.createFromBuffer(thing);
...
export class File_Icon_Image extends React.Component<
  T_File_Icon_Image_Props,
  { base64: ?string }
> {
  state = {
  ...
  render() {
    ...
    const { base64 } = this.state;
    const is_png_or_jpg =
      icon.type === "file" && /(\.png|\.jpe?g)$/.test(icon.path);
    ...
    if (icon.type === "file" && base64 == null && !is_png_or_jpg) {
      ...
      const source =
        icon.type === "file" && !is_png_or_jpg && base64 ? base64 : icon.path;
      if (type === "background") {
        return (
          <div
            style={{
  ...
  render() {
    ...
    return (
      <Flex
        style={style}
        children={children}
        innerRef={ref => (this.view_ref = ref)}
      />
    );
    // return (
