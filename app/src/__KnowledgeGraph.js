  ...
  return <MatchItem
    ...
    render_icon={<div
      style={{
        ...
        backgroundImage: item.image ? `url("${item.image.contentUrl}")` : '',
...
class KnowledgeGraph extends React.Component<{ search: string }, { result: ?any, movie_result: ?any }> {
  state = {
    result: null,
    movie_result: null,
  };
  ...
  async load(prevProps: { search: string }) {
    ...
    if (this.props.search !== prevProps.search) {
      ...
      if (search === this.props.search) {
        ...
        const movie_result_container = result.itemListElement && result.itemListElement[0] && result.itemListElement[0];
        if (movie_result_container && movie_result_container.resultScore > 20) {
          ...
          if (movie_result && movie_result['@type'].includes('Movie')) {
            const reponse = await fetch(`https://api.themoviedb.org/3/search/movie?api_key=${themoviedb_api_key}&query=${encodeURI(movie_result.name)}`)
            const movie_db = (await reponse.json()).results[0];

            if (movie_db) {
              this.setState({ movie_result: movie_db });
            }
            console.log(`movie_db:`, movie_db)
            // this.setState({
            //   movie_result:
            // })
          }
        ...
        <Flex column>
          <Flex row style={{ paddingLeft: 20, paddingRight: 20, paddingBottom: 20 }}>
            ...
            { is_movie &&
              <div
                style={{
                  ...
                  backgroundPosition: 'center',
                  backgroundImage: movie_result && movie_result.poster_path && `url("http://image.tmdb.org/t/p/w185${movie_result.poster_path}")`,
                  backgroundColor: !(movie_result && movie_result.poster_path) && `#c7c7c7`,
                }}
              />
            }
            ...
            <Flex column style={{ minWidth: 0 }}>
              <Flex style={{ fontSize: 32, flexWrap: 'wrap' }}>{item.name}</Flex>
              <Space height={5} />
              <Flex row  style={{ fontWeight: '100', fontSize: 16, flexWrap: 'wrap' }}>
              ...
              {item.detailedDescription && (
                <G>
                  ...
                  <Flex style={{ fontSize: 16 }}>
                    ...
                    <a href={item.detailedDescription.url}> { item.detailedDescription.url.includes('wikipedia') ? 'Wikipedia' : item.detailedDescription.url}</a></span>
                  </Flex>
                </G>
              )}
            </Flex>
          ...
          { (items || []).slice(1).map((item, i) => <KnowledgeGraph_Item key={i} item={item.result} />)}
        </Flex>
      );
    }
  }
